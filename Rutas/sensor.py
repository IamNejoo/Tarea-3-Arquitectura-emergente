...
from models import db, Sensor, Location, Company,  SensorData
...
def get_sensors():
    ...
    all_sensors = Sensor.query.join(Location).filter(Location.company_id == company.id).all()
...
def get_sensor(id):
    ...
    sensor = Sensor.query.join(Location).filter(Sensor.id == id, Location.company_id == company.id).first_or_404()
...
def delete_sensor(id):
    try:
        ...
        # Eliminar todos los datos relacionados del sensor
        SensorData.query.filter_by(sensor_id=sensor.id).delete()
        
        # Ahora eliminar el sensor
